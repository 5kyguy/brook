...
export default function viteSvgUsePlugin(): Plugin {
    ...
    function resolveSvg(root: string, importer: string, src: string) {
        // Handle Vite aliases
        if (src.startsWith('.')) {
            const besideImporter = path.resolve(path.dirname(importer), src);
            if (fs.existsSync(besideImporter)) {
                return normalizePath(besideImporter);
            }
            const inPublic = path.resolve(root, 'public', src.replace(/^\.\//, ''));
            if (fs.existsSync(inPublic)) {
                return normalizePath(inPublic);
            }
            return normalizePath(besideImporter);
        }
        // Check for alias
            ...
            for (const [_, { find, replacement }] of config.resolve.alias.entries()) {
                if (typeof find === 'string' ? src.startsWith(find) : find.test(src)) {
                    ...
                    if (path.isAbsolute(aliasedPath)) {
                        return normalizePath(aliasedPath);
                    }
